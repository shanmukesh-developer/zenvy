...
async function runSimulation() {
  try {
    ...
    const testUser = await User.create({
      name: "E2E Customer",
      phone: "111" + Math.floor(Math.random() * 10000000).toString().padStart(7, '0'),
    ...
    const testRider = await Rider.create({
      name: "E2E Rider",
      phone: "222" + Math.floor(Math.random() * 10000000).toString().padStart(7, '0'),
      password: "password123",
      vehicleType: "Bike",
      vehicleNumber: "AP00X" + Math.floor(Math.random() * 10000).toString().padStart(4, '0'),
      isOnline: true,
      isApproved: true
    });
    ...
    const res2 = await fetch(`${BASE_URL}/api/orders/${orderData._id}/restaurant-accept`, {
      method: 'PUT',
    ...
    const res3 = await fetch(`${BASE_URL}/api/delivery/accept/${orderData._id}`, {
      method: 'PUT',
    ...
    const res4 = await fetch(`${BASE_URL}/api/delivery/status/${orderData._id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${riderToken}` },
      body: JSON.stringify({ status: 'PickedUp' })
    });
    const riderPickedUpData = await res4.json();
    if (!res4.ok) throw new Error("Rider pickup failed: " + JSON.stringify(riderPickedUpData));
    console.log("-> Rider Marked Order as PickedUp");

    const res5 = await fetch(`${BASE_URL}/api/delivery/status/${orderData._id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${riderToken}` },
      body: JSON.stringify({ status: 'Delivered', pin: orderData.deliveryPin })
    });
    const riderDeliverData = await res5.json();
    if (!res5.ok) throw new Error("Rider deliver failed: " + JSON.stringify(riderDeliverData));
